...
interface PlanFormProps {
  isLoggedIn: boolean;
  isPro: boolean;
  esBienvenida?: boolean;        // ← nuevo: free con créditos de regalo
  creditosBienvenida?: number;   // ← cuántos le quedan
...
export const PlanForm = ({
  ...
  const Badge = () => {
    if (esBienvenida) return (
      <span className="flex items-center gap-1.5 text-xs font-semibold text-amber-300 bg-amber-300/10 border border-amber-300/20 px-3 py-1 rounded-full">
        ...
        {creditosBienvenida} créditos Pro de regalo
      </span>
    );
    if (isPro) return (
      <span className="flex items-center gap-1.5 text-xs font-semibold text-amber-300 bg-amber-300/10 border border-amber-300/20 px-3 py-1 rounded-full">
  ...
  const btnLabel = () => {
    ...
    if (isPro) return (
      <><Sparkles className="w-4 h-4 mr-2" />Generar Planificación Pro</>
    );
    return (
      <><Sparkles className="w-4 h-4 mr-2" />Generar Planificación Gratuita</>
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        <Field id="docente" label="Nombre del docente" placeholder="Ej. María González" value={data.docente} onChange={update("docente")} />
        <Field id="institucion" label="Institución" placeholder="Ej. Escuela Nro. 123" value={data.institucion} onChange={update("institucion")} />

        {/* Grado */}
        <div>
          <label htmlFor="grado" className="block text-cream text-sm font-medium mb-2">Año / Grado</label>
          <Select value={data.grado} onValueChange={(v) => setData((d) => ({ ...d, grado: v }))}>
            <SelectTrigger id="grado" className="w-full bg-ink-soft border border-ink-border text-cream rounded-xl px-4 h-12 outline-none focus:border-coral focus:ring-2 focus:ring-coral/30 transition-all duration-200 data-[placeholder]:text-cream/30">
            ...
            <SelectContent className="bg-ink border-ink-border text-cream max-h-72">
              {GRADO_GROUPS.map((group) => (
                <SelectGroup key={group.label}>
                  <SelectLabel className="text-cream/50 text-[10px] uppercase tracking-[0.2em] font-semibold">{group.label}</SelectLabel>
                  {group.options.map((opt) => (
                    <SelectItem key={opt} value={opt} className="text-cream focus:bg-coral/15 focus:text-cream cursor-pointer">{opt}</SelectItem>
                  ))}
                </SelectGroup>
              ))}
            </SelectContent>
        ...
        <Field id="materia" label="Materia / Asignatura" placeholder="Ej. Ciencias Naturales" value={data.materia} onChange={update("materia")} />

        {/* Fecha */}
        <div>
          <label htmlFor="fecha" className="block text-cream text-sm font-medium mb-2">Fecha de la clase</label>
          <input id="fecha" type="date" value={data.fecha} onChange={update("fecha")}
        ...
        {/* Duración */}
        <div>
          <label htmlFor="duracion" className="block text-cream text-sm font-medium mb-2">Duración de la clase</label>
          <Select value={data.duracion} onValueChange={(v) => setData((d) => ({ ...d, duracion: v }))}>
            <SelectTrigger id="duracion" className="w-full bg-ink-soft border border-ink-border text-cream rounded-xl px-4 h-12 outline-none focus:border-coral focus:ring-2 focus:ring-coral/30 transition-all duration-200 data-[placeholder]:text-cream/30">
              <SelectValue placeholder="¿Cuánto dura la clase?" />
            </SelectTrigger>
            <SelectContent className="bg-ink border-ink-border text-cream">
              {DURACIONES.map((d) => (
                <SelectItem key={d} value={d} className="text-cream focus:bg-coral/15 focus:text-cream cursor-pointer">{d} minutos</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>

        <Field id="tema" label="Tema de la clase" placeholder="Ej. La fotosíntesis" value={data.tema} onChange={update("tema")} className="sm:col-span-2" />

        {/* Objetivo */}
        <div className="sm:col-span-2">
          <label htmlFor="objetivo" className="block text-cream text-sm font-medium mb-2">Objetivo</label>
          <textarea id="objetivo" value={data.objetivo} onChange={update("objetivo")}
          ...
          <div className="sm:col-span-2">
            <label htmlFor="contexto" className="block text-sm font-medium mb-2 flex items-center gap-2">
              ...
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${
                ...
                {esBienvenida ? "🎁 REGALO" : "PRO"}
              </span>
            </label>
            <textarea id="contexto" value={data.contexto} onChange={update("contexto")}
      ...
      {isLoggedIn && (
        <p className="text-center text-cream/50 text-xs mt-4">
          {esBienvenida
            ? `${creditosBienvenida} planificación${creditosBienvenida !== 1 ? "es" : ""} Pro gratuita${creditosBienvenida !== 1 ? "s" : ""} restante${creditosBienvenida !== 1 ? "s" : ""} · Powered by Ohana IA`
            : isPro
            ? "Powered by Ohana IA"
...
const Field = ({
  ...
  <div className={className}>
    <label htmlFor={id} className="block text-cream text-sm font-medium mb-2">{label}</label>
